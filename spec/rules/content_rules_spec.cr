  ...
  describe "#run" do
    ...
    context "given a timeline with an object that has been deleted" do
      ...
      it "destroys the timeline entry" do
        subject.run(owner, delete_fresh)
        expect(Timeline.where(from_iri: owner.iri)).to be_empty
      end
    end
  end
