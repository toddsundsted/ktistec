...
module Rules
  abstract class View
    # The dislike notification view.
    #
    # There is one row per disliked object, holding the latest
      ...
      def stored_scope(key : Key) : {String, Array(DB::Any)}
        {
          ...
          Array(DB::Any){key[:from_iri], DISLIKE, key[:to_iri]},
        }
      end
    end

    register(Dislike.instance)
  end
end
