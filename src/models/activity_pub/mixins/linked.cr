        ...
        def self.dereference(key_pair, iri, *, ignore_cached = false, include_deleted = false, **options) : self
          if ignore_cached || (instance = self.find?(iri, include_deleted: include_deleted)).nil?
            if ::Ktistec::Model::Linked.local?(iri)
              ...
              Log.debug { "#{self}.dereference - #{iri} - skipping dereference for URL with fragment" }
              raise ::Ktistec::Model::NotFound.new("#{self} #{iri}: URL with fragment is not dereferenceable")
            else
              headers = HTTP::Headers{"Accept" => Ktistec::Constants::ACCEPT_HEADER}
              Ktistec::Network.get(key_pair, iri, headers) do |response|
                if Ktistec::Model::Linked.json_response?(response)
                  ...
                  if instance && !instance.iri_matches?(iri)
                    Log.warn { "#{self}.dereference - #{iri} - IRI mismatch: requested #{iri}, got #{instance.iri}" }
                    raise ::Ktistec::JSON_LD::Error.new("#{self}.dereference - #{iri} - IRI mismatch: requested #{iri}, got #{instance.iri}")
                  end
                else
                  Log.debug { "#{self}.dereference - #{iri} - non-JSON response (#{response.headers["Content-Type"]?})" }
                  raise ::Ktistec::JSON_LD::Error.new("#{self}.dereference - #{iri} - non-JSON response (#{response.headers["Content-Type"]?})")
                end
              end
            end
          end
                          ...
                          if ::Ktistec::Model::Linked.local?({{foreign_key}})
                            ...
                            Ktistec::Network.get(key_pair, {{foreign_key}}, headers) do |response|
                              if Ktistec::Model::Linked.json_response?(response)
                                ...
                                if {{name}}_ && !{{name}}_.iri_matches?({{foreign_key}})
                                  Log.warn { "#{self.class}##{{{name.stringify}}} - #{{{foreign_key}}} - IRI mismatch: requested #{{{foreign_key}}}, got #{{{name}}_.iri}" }
                                  raise ::Ktistec::JSON_LD::Error.new("#{self.class}##{{{name.stringify}}} - #{{{foreign_key}}} - IRI mismatch: requested #{{{foreign_key}}}, got #{{{name}}_.iri}")
                                else
                                  self.{{name}} = {{name}}_
                                end
                              else
                                Log.debug { "#{self.class}##{{{name.stringify}}} - #{{{foreign_key}}} -- non-JSON response (#{response.headers["Content-Type"]?})" }
                                raise ::Ktistec::JSON_LD::Error.new("#{self.class}##{{{name.stringify}}} - #{{{foreign_key}}} -- non-JSON response (#{response.headers["Content-Type"]?})")
                              end
                            end
                          end
                        else
