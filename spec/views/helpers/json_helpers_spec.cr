...
Spectator.describe "helpers" do
  ...
  describe "activity_pub_collection" do
    ...
    let(collection) do
      Ktistec::Util::PaginatedArray(ViewHelperSpecSupport::Model).new.tap do |c|
    ...
    context "with an empty collection" do
      let(raw_json) { String.build { |content_io| activity_pub_collection(collection) } } # ameba:disable Lint/UnusedArgument

      it "does not emit any page markers" do
        expect(raw_json).not_to match(/\?page=|"page":/)
      end

      context "on a paged request" do
        let(query) { "?max_id=100" }
